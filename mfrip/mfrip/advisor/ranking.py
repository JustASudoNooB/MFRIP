...
def _norm(vals: list[float], higher_better=True) -> list[float]:
    """Percentile rank of each value within the peer set (0 = worst, 1 = best).

    Rank-based rather than min-max: min-max lets a single extreme fund stretch
    the scale and squash everyone else's scores together, while percentile
    ranks are robust to outliers. This is also how Value Research and
    Morningstar place a fund within its category. Ties share the average rank.
    """
    s = pd.Series(vals, dtype=float)
    if s.max() - s.min() < 1e-12:
        return [0.5] * len(vals)
    r = s.rank(method="average", ascending=higher_better)
    return ((r - 1.0) / (len(vals) - 1.0)).tolist()
